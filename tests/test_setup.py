...
class TestInit(unittest.TestCase):
    def test_writes_config_env_workflow(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            wf = (root / ".github" / "workflows" / "cascade.yml").read_text()
            self.assertIn("create-github-app-token", wf)
            self.assertIn("CASCADE_GITHUB_APP_ID", wf)
            self.assertIn("CASCADE_GITHUB_TOKEN", wf)
            self.assertTrue(any("GitHub App" in n for n in notes))
