...
class TestComment(unittest.TestCase):
    ...
    def test_comments_do_not_embed_logo_in_markdown(self):
        report = json.loads(GOLDEN_REPORT.read_text())
        report["remediations"][1]["rewritten_sql"] = GOLDEN_SQL.read_text()
        for md in (
            build_pr_comment(report),
            build_pr_comment({"source_urn": "urn:x", "changes": [], "downstream": []}),
            build_stack_comment("https://github.com/o/r/pull/9"),
            build_remediation_pr_body(report),
        ):
            self.assertNotIn("<img", md)
            self.assertNotIn("apple-touch-icon", md)
            self.assertNotIn("logo.png", md)
...
class TestGitHubAct(unittest.TestCase):
    def test_dry_run_writes_comment_without_token(self):
        with tempfile.TemporaryDirectory() as tmp:
            env = {
                ...
                if k not in ("GITHUB_TOKEN", "CASCADE_GITHUB_TOKEN")
            }
    ...
    def test_bot_pat_posts_when_actions_token_absent(self):
        calls: list[tuple[str, str]] = []

        def fake_api(method: str, path: str, body=None):
            calls.append((method, path))
            if method == "POST" and path == "/issues/3/comments":
                return {"id": 12, "html_url": "https://example/c/12"}
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            env["CASCADE_GITHUB_TOKEN"] = "bot-pat"
...
class TestApply(unittest.TestCase):
    ...
    def test_run_apply_dry_run_end_to_end(self):
        ...
        report["remediations"][1]["rewritten_sql"] = GOLDEN_SQL.read_text()
        with tempfile.TemporaryDirectory() as tmp:
            env = {k: v for k, v in os.environ.items() if k not in ("GITHUB_TOKEN", "CASCADE_GITHUB_TOKEN", "CASCADE_WRITEBACK", "CASCADE_OPEN_DOWNSTREAM_PR")}
            with mock.patch.dict(os.environ, env, clear=True):
